...
**External-managed** resources—`codebase-design`, `wayfinder`, `grill-with-docs`, `grill-me`, `grilling`, `writing-for-agents`, and `ui-ux-pro-max`—are acquired during setup/update from immutable identities in the packaged lock metadata. Their runtime bytes are not bundled in ai-config.
