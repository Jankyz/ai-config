...
- **Built registry**: generated JSON files served to CLI consumers, usually
  from `public/r`. Use `PROJECT_OWNED_SHADCN build` to create this form.
...
```bash
PROJECT_OWNED_SHADCN build
PROJECT_OWNED_SHADCN build registry.json --output public/r
```

Use CLI commands to inspect the result:

```bash
PROJECT_OWNED_SHADCN list @acme
PROJECT_OWNED_SHADCN search @acme -q "login"
PROJECT_OWNED_SHADCN view @acme/login-form
PROJECT_OWNED_SHADCN add @acme/login-form --dry-run
PROJECT_OWNED_SHADCN registry validate ./registry.json
```

Use GitHub addresses directly for public GitHub registries:

```bash
PROJECT_OWNED_SHADCN list owner/repo
...
PROJECT_OWNED_SHADCN registry validate owner/repo
```
