...
MCP access is optional external evidence. Do not start a server, write an editor configuration, inspect credentials, or configure provider MCP as part of this skill. If the owner has already configured a compatible project MCP server, its read-only registry tools may be used within the approved project scope.

---

## Tools

> **Tip:** MCP tools handle registry operations (search, view, install). For project configuration (aliases, framework, Tailwind version), use `PROJECT_OWNED_SHADCN info` — there is no MCP equivalent.
