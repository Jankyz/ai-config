...
2. Manually publish that exact reviewed tarball as `@jankyz/ai-config@0.1.0` with the owner's npm account and 2FA: `npm publish <reviewed-tarball-path> --access public`. The `--access public` flag is required for this public scoped package.
...
5. The workflow verifies `v0.1.0`, recognizes `@jankyz/ai-config@0.1.0` as already published, and completes without a duplicate publish.
