...
`aic-research`, `aic-clarify`, `aic-analyze`, `aic-diagnose`, `aic-review`, `aic-verify`, and `aic-security-review` are read-only. `aic-plan` creates a plan but does not implement. `aic-implement` mutates only under an approved plan. `aic-lore-commit` may make a Git commit only after explicit invocation and owner authorization.
